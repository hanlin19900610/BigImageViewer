package com.mufeng.library.view.helper;
...
import com.mufeng.library.view.nine.ViewHelper;
 ...
 */
public class FingerDragHelper extends ConstraintLayout  {
    ...
    private SubsamplingScaleImageViewDragClose imageView;  //原图
    private PhotoView thumbnailImageView;   //缩略图
    private PhotoView gifImageView;  //gif
    ...
    protected void onFinishInflate() {
        ...
        gifImageView = (PhotoView) getChildAt(1);
        thumbnailImageView = (PhotoView) getChildAt(2);
    }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (ImagePreview.getInstance().isEnableDragClose()) {
                    if (gifImageView != null && gifImageView.getVisibility() == View.VISIBLE) {
                        isIntercept = (gifImageView.getScale() <= (gifImageView.getMinimumScale() + 0.001F))
                                && (gifImageView.getMaxTouchCount() == 0 || gifImageView.getMaxTouchCount() == 1)
                        ...
                        isIntercept = (imageView.getScale() <= (imageView.getMinScale() + 0.001F))
                                ...
                                && Math.abs(ev.getRawY() - mDownY) > 2 * mTouchslop
                                && imageView.atYEdge;
                    } else if (thumbnailImageView != null && thumbnailImageView.getVisibility() == VISIBLE) {
                        isIntercept = (thumbnailImageView.getScale() <= (thumbnailImageView.getMinimumScale() + 0.001F))
                                && (thumbnailImageView.getMaxTouchCount() == 0 || thumbnailImageView.getMaxTouchCount() == 1)
                                && Math.abs(ev.getRawY() - mDownY) > 2 * mTouchslop;
                    }
                }
                break;
